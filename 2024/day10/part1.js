...
function hike(x, y, visited = new Set()) {
    const key = `${x},${y}`; 
    ...
    steps.forEach(step => {
        ...
        if (is_valid_step(x, y, new_x, new_y)) result = result.concat(hike(new_x, new_y, visited));
    });
    return result;
}
...
function tailhead_score(x, y) {
    ...
    steps.forEach(step => {
        const new_x = x + step[0];
        const new_y = y + step[1];
        if (is_valid_step(x, y, new_x, new_y)) count += hike(new_x, new_y, visited).length;
    });
